...
class TestSecurityComplete:
        ...
        try:
            ...
            for dangerous_input in dangerous_inputs:
                try:
                    ...
                    protection_results.append(
                        {"input": dangerous_input, "error": str(e), "protected": False}
                    )

            protected_count = sum(
                1 for r in protection_results if r.get("protected", False)
            )

            return {
                "success": protected_count
                >= len(dangerous_inputs) * 0.8,  # 80% should be protected
    ...
    def test_pii_scrubbing_functionality(self):
        ...
        try:
            ...
            test_cases = [
                ...
                {"input": "Normal text without sensitive data", "should_scrub": []},
            ]
                ...
                try:
                    ...
                    for pattern in test_case["should_scrub"]:
                        ...
                        patterns_found.append(
                            {"pattern": pattern, "still_present": still_present}
                        )
                    ...
                    scrubbing_results.append(
                        {
                            ...
                            "scrubbed_correctly": not any(
                                p["still_present"] for p in patterns_found
                            ),
                        }
                    )
            ...
            success_count = sum(
                1 for r in scrubbing_results if r.get("scrubbed_correctly", False)
            )

            return {
                "success": success_count
                >= len(test_cases) * 0.75,  # 75% should work correctly
        ...
        try:
            ...
            for domain in valid_domains + invalid_domains:
                try:
                    ...
                    domain_results.append(
                        {"domain": domain, "error": str(e), "correct": False}
                    )
                ...
                try:
                    if hasattr(string_validator, "validate"):
                        ...
                        string_results.append(
                            {
                                "string": str(test_string)[:20] + "..."
                                if test_string and len(str(test_string)) > 20
                                else str(test_string),
                                "valid": is_valid,
                            }
                        )
                    else:
                        string_results.append(
                            {"string": "validator_method_missing", "valid": False}
                        )
                except Exception as e:
                    string_results.append(
                        {
            ...
            string_valid = len(
                [r for r in string_results if r.get("valid", False) is not False]
            )

            return {
                "success": domain_correct
                >= len(valid_domains),  # At least valid domains should work
    ...
    def test_security_audit_functionality(self):
        ...
        try:
            ...
            try:
                audit_authentication("test_user", "test_action")
                audit_results["authentication"] = True
            except Exception:
                audit_results["authentication"] = False

            try:
                audit_cli_usage("test_command", {"test": "params"})
    ...
    def test_usage_monitoring(self):
        ...
        try:
            from src.utils.usage_monitor import (
    ...
    def test_notification_system(self):
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"Notification system test failed: {e}",
            }
