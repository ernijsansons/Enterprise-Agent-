...
class TestCommandInjectionPrevention:
    ...
    def claude_provider(self, mock_subprocess):
        ...
        with patch.object(ClaudeCodeProvider, 'verify_cli_available', return_value=True), \
             patch.object(ClaudeCodeProvider, 'verify_subscription_auth', return_value=True), \
             patch.object(ClaudeCodeProvider, '_load_persistent_sessions'):
    ...
    def test_claude_provider_prevents_shell_injection(self, claude_provider, mock_subprocess):
        ...
        for injection in injection_attempts:
            ...
            with patch.object(claude_provider, '_enhance_prompt_with_context', return_value="test prompt"):
                try:
            ...
            if mock_subprocess.called:
                ...
                assert call_kwargs.get('shell', False) == False, \
    ...
    def test_working_directory_validation(self, claude_provider, mock_subprocess):
        ...
        with patch.object(claude_provider, '_enhance_prompt_with_context', return_value="test"):
            try:
    ...
    @pytest.mark.parametrize("injection_in_prompt", [
        "'; DROP DATABASE; --",
        "$(rm -rf /)",
    ...
    def test_prompt_injection_prevention(self, claude_provider, mock_subprocess, injection_in_prompt):
        """Test that malicious prompts don't cause command injection."""
        with patch.object(claude_provider, '_enhance_prompt_with_context', return_value=injection_in_prompt):
            try:
                claude_provider.call_model(
                    prompt=injection_in_prompt,
                    model="sonnet"
                )
            except Exception:
                pass
        ...
        if mock_subprocess.called:
            ...
            assert not any(char in ' '.join(command_parts) for char in [';', '|', '$', '`']), \
    ...
    def test_no_direct_environment_manipulation(self):
        ...
        try:
            ...
            assert result == False, "Should detect API key presence"
    ...
    def test_command_list_construction(self, claude_provider):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            with patch.object(claude_provider, '_enhance_prompt_with_context', return_value="test"):
                with patch.object(claude_provider, 'verify_cli_available', return_value=True):
                    try:
    ...
    def test_no_shell_glob_expansion(self, claude_provider, mock_subprocess):
        ...
        for pattern in glob_patterns:
            ...
            with patch.object(claude_provider, '_enhance_prompt_with_context', return_value=pattern):
                try:
            ...
            if mock_subprocess.called:
                ...
                assert pattern in cmd or pattern in str(cmd), \
...
class TestAuthManagerSecurity:
    ...
    def test_no_api_key_deletion(self):
        ...
        try:
            ...
            assert os.environ.get('ANTHROPIC_API_KEY') == 'test_api_key_123', \
    ...
    def test_safe_file_operations(self, tmp_path):
        ...
        assert result == True, "Should detect API key in file"
    ...
    def test_login_command_safety(self, mock_run):
        ...
        assert mock_run.call_args.kwargs.get('shell', False) == False
