...
def test_rate_limiter_basic():
    ...
    from src.utils.rate_limiter import RateLimitConfig, TokenBucket, RateLimiter

    # Test token bucket
    config = RateLimitConfig(max_tokens=5, refill_rate=2.0)
    bucket = TokenBucket(config)

    assert bucket.acquire(3) == True, "Should acquire 3 tokens"
    assert bucket.acquire(3) == False, "Should not acquire 3 more tokens"
    assert bucket.acquire(2) == True, "Should acquire remaining 2 tokens"

    # Test rate limiter
    limiter = RateLimiter()
    limiter.add_limit("test", config)

    assert limiter.acquire("test", 2) == True, "Should acquire tokens"
    status = limiter.get_status("test")
    assert "tokens_available" in status, "Should have status"

def test_circuit_breaker_basic():
    """Test basic circuit breaker functionality."""
    from src.utils.circuit_breaker import CircuitBreaker, CircuitBreakerConfig, CircuitState, CircuitBreakerError
